...
// duplicate this for now as will extend in future release 
        m_getCustomerAllFields = new PreparedSentence(s, "SELECT ID FROM customers WHERE SEARCHKEY=? AND NAME=? ", new SerializerWriteBasic(new Datas[]{
            Datas.STRING, Datas.STRING}), customerIdRead
        );

        m_getCustomerSearchKeyAndName = new PreparedSentence(s, "SELECT ID FROM customers WHERE SEARCHKEY=? AND NAME=? ", new SerializerWriteBasic(new Datas[]{
            Datas.STRING, Datas.STRING}), customerIdRead
        );

        m_getCustomerBySearchKey = new PreparedSentence(s, "SELECT ID FROM customers WHERE SEARCHKEY=? ", SerializerWriteString.INSTANCE, customerIdRead
        );
        ...
        m_closedcash = new StaticSentence(s, "SELECT HOST, HOSTSEQUENCE, DATESTART, DATEEND, NOSALES "
                + "FROM closedcash WHERE HOSTSEQUENCE = ?", SerializerWriteString.INSTANCE, new SerializerReadBasic(new Datas[]{
            Datas.STRING,
            Datas.INT,
            Datas.TIMESTAMP,
            Datas.TIMESTAMP,
...
//  Push Products into CSVImport table      
        m_insertCSVEntry = new StaticSentence(s, "INSERT INTO csvimport ( "
...
//  Push Product Quantity Update into CSVImport table      
        m_insertStockUpdateEntry = new StaticSentence(s, "INSERT INTO csvimport ( "
                + "ID, ROWNUMBER, CSVERROR, REFERENCE, "
                + "CODE, PRICEBUY ) "
                + "VALUES (?, ?, ?, ?, ?, ?)", new SerializerWriteBasic(new Datas[]{
            Datas.STRING,
...
//  Push Customers into CSVImport table      
        m_insertCustomerCSVEntry = new StaticSentence(s, "INSERT INTO csvimport ( "
        ...
        m_addOrder = new StaticSentence(s, "INSERT INTO orders (ORDERID, QTY, DETAILS, ATTRIBUTES, "
        ...
        m_updateOrder = new StaticSentence(s, "UPDATE orders SET "
                + "ORDERID = ?, "
                + "QTY = ?, "
                + "DETAILS = ?, "
                + "ATTRIBUTES = ?, "
                ...
                + "AUXILIARY = ?, "
                + "COMPLETETIME = ? "
                + "WHERE ORDERID = ? ", new SerializerWriteBasic(new Datas[]{
            Datas.STRING, // OrderId
            Datas.INT, // Qty
            Datas.STRING, // Details
            Datas.STRING, // Attributes
        ...
        m_deleteOrder = new StaticSentence(s, "DELETE FROM orders WHERE ORDERID = ?", SerializerWriteString.INSTANCE);
